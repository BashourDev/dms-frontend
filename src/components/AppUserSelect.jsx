...
export default function AppUserSelect({
  ...
  return (
    <div
      className={`flex flex-col text-dark text-xs lg:text-sm mt-5 mb-1 ${className}`}
    >
      <label>{label}</label>
      <Listbox value={value} onChange={(value) => handleChange(value)}>
        <div className="relative mt-1">
          <Listbox.Button className="relative w-full py-2 pr-6 pl-10 text-left bg-inherit border-[1px] border-lightGray rounded-lg shadow-md cursor-default focus:outline-none focus-visible:ring-2 focus-visible:ring-opacity-75 focus-visible:ring-white focus-visible:ring-offset-sky-300 focus-visible:ring-offset-2 focus-visible:border-indigo-500 sm:text-sm">
            <span className="block truncate text-right">{value?.username}</span>
          ...
          <Transition
            ...
            leaveTo="opacity-0"
          >
            <Listbox.Options className="absolute w-full py-1 mt-1 overflow-auto text-xs lg:text-sm bg-white rounded-md shadow-lg max-h-60 ring-1 ring-black ring-opacity-5 focus:outline-none sm:text-sm">
